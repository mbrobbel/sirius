          ...
          "[mgpu][operator-mgpu][hash_join][gpu_execution]")
{
  ...
  // Both GPUs saw SOME pipeline work. Under the GPU_SCAN architecture,
  // parquet reads dispatch as
  // gpu_pipeline_task entries (logged via [mgpu-audit] pipeline_task
  // dispatched) rather than scan_batch entries.
  // BUILD_PROBE pins post-partition probe tasks to one GPU via SCHED-00,
  // but the scan pipelines upstream of the join are not partition-pinned
  // and distribute across both GPUs, so
          ...
          "[mgpu][operator-mgpu][hash_join][stress][followup-17][gpu_execution]")
{
  ...
  // GPU_SCAN architecture, not as scan_executor scan_batch entries.
  // cache=table_gpu also collapses scan_executor traffic on warm iterations.
  // See BUILD_PROBE comment above.
  REQUIRE(counts[0].pipeline_ids.size() >= 1);
  REQUIRE(counts[1].pipeline_ids.size() >= 1);
