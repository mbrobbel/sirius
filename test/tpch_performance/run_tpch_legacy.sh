#!/usr/bin/env bash
# =============================================================================
# Run TPC-H benchmark on legacy Sirius (gpu_processing).
#
...
# Defaults
if [ -z "$DB_PATH" ]; then
    DB_PATH="$PROJECT_DIR/test_datasets/tpch_sf${SF}.duckdb"
fi
...
if [ -z "$OUTPUT_DIR" ]; then
    OUTPUT_DIR="$PROJECT_DIR/tpch_results_sf${SF}"
fi
mkdir -p "$OUTPUT_DIR"
...
if [ ! -f "$DB_PATH" ]; then
    ...
    echo "  $DUCKDB $DB_PATH -c \"INSTALL tpch; LOAD tpch; CALL dbgen(sf=$SF);\""
    exit 1
fi
