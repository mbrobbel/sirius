...
unique_ptr<TableFilterSet> GPUCreateTableFilterSet(TableFilterSet& table_filters,
...
//     // 	    "Mismatch between input table types and child node types - expected %llu but got
//     %llu",
...
//     // child_node->types[proj_idx]; 	if (child_node->types[proj_idx] != target_type) {
...
//     // 	auto proj = make_uniq<PhysicalProjection>(std::move(return_types),
...
//     // auto node = make_uniq<PhysicalTableInOutFunction>(op.types, op.function,
//     // std::move(op.bind_data), column_ids,
...
//     throw InternalException("LogicalGet::project_input can only be set for table-in-out
//     functions");
//   }
...
//       }
//       filter = make_uniq<GPUPhysicalFilter>(
...
//     auto node = make_uniq<GPUPhysicalTableScan>(op.returned_types,
//                                                 op.function,
...
//       }
//     }
//     unique_ptr<GPUPhysicalProjection> projection = make_uniq<GPUPhysicalProjection>(
...
unique_ptr<GPUPhysicalOperator> GPUPhysicalPlanGenerator::CreatePlan(LogicalGet& op)
{
  ...
  if (!op.children.empty()) {
    throw NotImplementedException("Table Input Output functions are not supported yet");
    // reference<GPUPhysicalOperator> child = ResolveAndPlan(std::move(op.children[0]));
    ...
    // if (any_cast_required) {
    // 	auto &proj = Make<GPUPhysicalOperator>(std::move(return_types), std::move(expressions),
  ...
  unique_ptr<GPUPhysicalOperator> filter;
  auto& projection_ids = op.projection_ids;
  ...
  if (!op.function.projection_pushdown) {
    // function does not support projection pushdown
    // auto &table_scan = Make<GPUPhysicalTableScan>(
    //     op.returned_types, op.function, std::move(op.bind_data), op.returned_types, column_ids,
    //     vector<column_t>(), op.names, std::move(table_filters), op.estimated_cardinality,
