...
struct str_top_n_record_type {
  ...
  __host__ __device__ str_top_n_record_type(uint32_t _row_id, uint32_t _key_prefix)
...
__global__ void create_top_n_records(str_top_n_record_type* records,
                                     uint32_t* indexes,
                                     uint8_t* col_chars,
                                     uint64_t* col_offsets,
                                     uint32_t num_records)
{
  ...
  if (row_id < num_records) {
    ...
    uint32_t curr_record_prefix = 0;
    uint32_t curr_shift         = 3 * BITS_IN_BYTE;
...
__global__ void top_groups_calculator(uint32_t* group_starting_offsets,
                                      uint32_t num_results_needed,
                                      uint32_t* num_records_to_consider)
{
  uint32_t group_id = 0;
                                              ...
                                              uint64_t num_records)
{
  ...
  if (curr_record < num_records) {  // Determine the record length from the offsets buffer
    ...
    // Set the value of the last string to zero so therefore it will populate the last offset
    // properly
...
void CustomStringTopN(vector<shared_ptr<GPUColumn>>& keys,
                      ...
                      idx_t num_results)
{
  ...
  uint8_t* col_chars            = col_data.data;
  uint64_t* col_offsets         = col_data.offset;
  ...
  str_top_n_record_type* d_records = reinterpret_cast<str_top_n_record_type*>(
    gpuBufferManager->customCudaMalloc<string_top_n_record_type>(num_records, 0, 0));
  create_top_n_records<<<num_create_workers, BLOCK_THREADS>>>(
    d_records, d_indexes, col_chars, col_offsets, num_records);
  ...
  if (order_by_type[0] == OrderByType::ASCENDING) {
    ...
    d_prefix_sort_temp_storage =
      gpuBufferManager->customCudaMalloc<uint8_t>(prefix_sort_temp_storage_bytes, 0, 0);
    cub::DeviceMergeSort::SortKeys(d_prefix_sort_temp_storage,
                                   prefix_sort_temp_storage_bytes,
                                   d_records,
                                   num_records,
  ...
  // We only need to sort the groups whose starting row id < N that we need as the resulting records
  // might be in the last row of that group. In order to do that we first need to perform a
  // reduction to get group boundaries and then use that to determine how many of the initial groups
  // do we actually need to sort
  START_TIMER();
  CustomTopNMin reduce_min_operator;
  str_top_n_record_type* d_result_groups = reinterpret_cast<str_top_n_record_type*>(
    gpuBufferManager->customCudaMalloc<string_top_n_record_type>(num_records, 0, 0));
  ...
  cub::DeviceReduce::ReduceByKey(d_key_reduction_temp_storage,
                                 key_reduction_temp_storage_bytes,
                                 d_records,
                                 d_result_groups,
                                 d_indexes,
                                 d_group_boundaries,
                                 ...
                                 reduce_min_operator,
                                 num_records);
  d_key_reduction_temp_storage =
    gpuBufferManager->customCudaMalloc<uint8_t>(key_reduction_temp_storage_bytes, 0, 0);
  cub::DeviceReduce::ReduceByKey(d_key_reduction_temp_storage,
                                 key_reduction_temp_storage_bytes,
                                 d_records,
                                 d_result_groups,
  ...
  if (order_by_type[0] == OrderByType::ASCENDING) {
    ...
    cub::DeviceMergeSort::SortKeys(d_record_sort_temp_storage,
                                   record_sort_temp_storage_bytes,
                                   d_records,
                                   num_complete_sort_records,
                                   string_less_than_cmp);
    d_record_sort_temp_storage =
      gpuBufferManager->customCudaMalloc<uint8_t>(record_sort_temp_storage_bytes, 0, 0);
    cub::DeviceMergeSort::SortKeys(d_record_sort_temp_storage,
                                   record_sort_temp_storage_bytes,
                                   d_records,
                                   num_complete_sort_records,
  ...
  uint8_t* project_col_chars    = project_col_data.data;
  uint64_t* project_col_offsets = project_col_data.offset;
  ...
  materialize_determine_lengths<<<num_materialize_worker, BLOCK_THREADS>>>(
    d_records, project_col_offsets, d_new_offsets, num_results);

  void* d_prefix_sum_temp_storage      = nullptr;
  size_t prefix_sum_temp_storage_bytes = 0;
  cub::DeviceScan::ExclusiveSum(d_prefix_sum_temp_storage,
                                prefix_sum_temp_storage_bytes,
                                d_new_offsets,
                                d_new_offsets,
                                num_results + 1);
  d_prefix_sum_temp_storage = reinterpret_cast<void*>(
    gpuBufferManager->customCudaMalloc<uint8_t>(prefix_sum_temp_storage_bytes, 0, 0));
  cub::DeviceScan::ExclusiveSum(d_prefix_sum_temp_storage,
                                prefix_sum_temp_storage_bytes,
                                d_new_offsets,
                                d_new_offsets,
  ...
  cudaMemcpy(
    &num_total_bytes, d_new_offsets + num_results, sizeof(uint64_t), cudaMemcpyDeviceToHost);
  uint8_t* d_result_chars = gpuBufferManager->customCudaMalloc<uint8_t>(num_total_bytes, 0, 0);
  materialize_copy_string<<<num_materialize_worker, BLOCK_THREADS>>>(
    d_records, project_col_chars, project_col_offsets, d_result_chars, d_new_offsets, num_results);

  // Set the new column
  GPUColumnType project_col_type = project_col_data.type;
  bool is_str_col                = project_col_data.type.id() == GPUColumnTypeId::VARCHAR;
  // TODO: FOR DEVESH, WE NEED TO MAKE SURE THAT THE VALIDITY MASK IS ADDED HERE, RIGHT NOW WE
  // ASSUME IT'S ALL VALID
  auto validity_mask = createNullMask(num_results, cudf::mask_state::ALL_VALID);
  projection[0]      = make_shared_ptr<GPUColumn>(num_results,
                                             project_col_type,
                                             d_result_chars,
                                             d_new_offsets,
                                             num_total_bytes,
                  ...
                  idx_t num_results)
{
  ...
  if (Config::USE_CUSTOM_TOP_N) {
    bool use_customed_implementation =
      num_results > 0 && num_keys == num_projections && num_keys == 1;
    for (size_t col = 0; col < num_keys; col++) {
      use_customed_implementation = use_customed_implementation &&
                                    keys[col]->data_wrapper.type.id() == GPUColumnTypeId::VARCHAR;
    }
    for (size_t col = 0; col < num_projections; col++) {
      use_customed_implementation =
        use_customed_implementation &&
        projection[col]->data_wrapper.type.id() == GPUColumnTypeId::VARCHAR;
    }
    SIRIUS_LOG_DEBUG(
  ...
  for (int col = 0; col < num_projections; col++) {
    ...
    projection[col]->setFromCudfColumn(
      sorted_column, projection[col]->is_unique, nullptr, 0, gpuBufferManager);
  }
