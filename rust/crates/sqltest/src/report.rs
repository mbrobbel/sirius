...
impl Report {
    pub fn new(provenance: Provenance) -> Self {
        Self {
            ...
            provenance,
            execution_evidence:
                "GPU execution unverified; Sirius queries run with fallback disabled".into(),
            cases: Vec::new(),
        }
    }
    ...
    pub fn markdown(&self, previous: Option<&Report>) -> String {
        ...
        output.push_str("\n## Suites\n\n| Suite | Configuration | Matches | Total | Empty reference results |\n| --- | --- | ---: | ---: | ---: |\n");
        let mut suites = BTreeMap::<_, (usize, usize, usize)>::new();
        for case in &self.cases {
            let suite = case.suite.as_str();
            let entry = suites.entry((suite, case.configuration())).or_default();
            entry.0 += usize::from(case.outcome == Outcome::Match);
            entry.1 += 1;
            entry.2 += usize::from(case.reference_rows == Some(0));
        }
        for ((suite, configuration), (matches, total, empty)) in suites {
