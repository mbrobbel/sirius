...
A parquet split's static filter is a `shared_ptr<duckdb::Expression>` carried by the parquet ingestible's post-filter/projection metadata. During split production, `parquet_gpu_ingestible` calls `gpu_expression_translator::translate_expression_with_names` to lower it to a cuDF AST. The translation outcome is what the dynamic-filter merge has to interleave with:
...
**Consumer:** parquet GPU ingestible split production.
