...
- **Per-operator `[mgpu]` coverage** — the SCHED-RR contract for the colocation-sensitive operators is exercised on 2 GPUs by the dedicated tests `test_physical_order_mgpu.cpp`, `test_physical_hash_join_mgpu.cpp` (incl. a Q11-like `BUILD_PROBE` case), and `test_physical_grouped_aggregate_merge_mgpu.cpp`, plus the `[TPC-H]` 2-GPU cases (`RUN_TPCH_MGPU`), each asserting CPU-baseline match. (A former Phase-15 `test_mgpu_stress.cpp` re-ran five of these query shapes under 100 `_no_pref_rr_counter` offsets; it was removed once `_no_pref_rr_counter` was deprecated — no-preference distribution now arises from device-ready signal ordering, not that counter — leaving the 100× loop as redundant probabilistic stress over shapes the dedicated tests already cover.)
