...
- **Ungrouped COUNT(DISTINCT):** Supports one `COUNT(DISTINCT column)` over a BIGINT or VARCHAR column, including upstream WHERE filters. Local tasks emit a LIST of unique non-NULL values; the global merge counts unique values across those lists and returns BIGINT. Empty input returns zero; empty strings count as values. Mixed aggregates, multiple DISTINCT aggregates, expression/tuple arguments, and aggregate FILTER/ORDER BY modifiers remain unsupported. The concatenated distinct state and merge workspace must fit one GPU task; this path does not implement partitioned out-of-core deduplication.
